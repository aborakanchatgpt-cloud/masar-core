...
- ⚠️ **n8n Cloud شبه منتهٍِ** (تجربة 1000 تنفيذ؛ بقي القليل). أوقفنا الوركفلو `CjlFHF1JG3GzYLbz` (مراقب كل 15 دقيقة) ومهمتي Discovery وPipeline Watchdog القديمتين مؤقتًا لحماية الرصيد. **الجسر البديل:** Masar MCP bridge داخل Core (`/mcp/<token>`) — انظر القسم 4.
...
### B3 — WIP (منفّذ محليًا، فحص حي جزئي 2026-09-08، بانتظار معيار الحمل الرقمي) — المطابقة والملف والمحفظة (المرحلة 3)
...
2. جامع جديد لمصادر سعودية مغلقة فعليًا (`sitemap_jsonld`/RSS لمواقع توظيف حكومية سعودية كطاقات أو جدارة إن وُجد RSS رسمي، أو صفحات وظائف شركات سعودية كبرى بـJSON-LD مضمّن) بدل الاستمرار بإضافة شركات Greenhouse عالمية هامشية (معدّل نجاح الدفعات السابقة يتراجع بسرعة: 13→8→6→11 مرشّحًا أعطوا 5→1→0→1 مصادر ناجية فقط) — **لا يزال TODO**.
