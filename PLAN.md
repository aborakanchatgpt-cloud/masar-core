...
> آخر تحديث: 6 سبتمبر 2026 — كاتبه: المراجع (Fable). هذا الملف هو **مصدر الحقيقة الوحيد** لحالة المشروع. المرجع التفصيلي للتصميم: `docs/EXECUTION_GUIDE.md`.
...
|---|---|---|---|
| **EXECUTOR** (المنفّذ) | جلسة مجدولة بنموذج Sonnet | كل 3 ساعات | يقرأ هذا الملف، يأخذ **أول بند بحالة `TODO` في قسم "التالي"**، ينفّذه كاملًا مع اختباره، يرفع الملفات، يحدّث القسم 2 و3 هنا، ويتوقف. بند واحد فقط في التشغيلة. |
| **REVIEWER** (المراجع) | جلسة مجدولة بالنموذج الافتراضي (Fable) | بعد كل منفّذ بساعة | يتحقق من آخر بند `DONE` مقابل معيار قبوله فعليًا (فحص حي عبر n8n → Core، قراءة الكود من GitHub)، يعيده إلى `TODO` مع تصحيحات إن فشل، يرتّب البنود التالية، يحدّث القسم 1، ويرسل تقريرًا قصيرًا لأحمد مساءً فقط. |
...
2. **قفل:** قبل البدء اكتب في القسم 5 سطر `LOCK: <الدور> <وقت UTC>`; وعند الانتهاء احذفه. إن وجدت قفلًا عمره أقل من 90 دقيقة، توقف دون عمل.
3. **حجم التشغيلة:** بند واحد. إن لم يكتمل، اكتب حالته `WIP` مع ما أُنجز بدقة وما تبقى، حتى تكمله التشغيلة التالية من حيث توقفت.
...
5. **متى تُبلغ أحمد (عبر `ZiVlsBibVps3pxGB` body `{"message": "..."}`):** سر يجب أن يدخله، دفع، إطفاء القديم، أو فشل متكرر ثلاث مرات لنفس البند. غير ذلك: المراجع يرسل تقريرًا واحدًا مساءً.
...
- ⚠️ **n8n Cloud تجريبي ينتهي ~10 سبتمبر 2026** (785/1000 تنفيذ مستهلكة). البند B0 يحسم النقل.
- ⏳ بانتظار أحمد (مرة واحدة): اعتماد `Masar GitHub` في n8n (Header Auth: `Authorization: Bearer <github token>`) ليعمل وركفلو الرفع؛ واسم نموذج Sonnet لضبط مهمة المنفّذ.
...
- **GitHub - Commit File** (يُنشأ بعد اعتماد `Masar GitHub`): body: `{"path": "core/app/x.py", "content": "<نص الملف كاملًا>", "message": "..."}` → ينشئ/يحدّث الملف في `main` (يجلب sha تلقائيًا). للملفات الكبيرة أرسلها واحدًا واحدًا.
